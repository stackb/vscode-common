...
import { isPromiseCanceledError } from '../errors';
import { URI } from '../uri';

describe('Async', () => {

	it('cancelablePromise - set token, don\'t wait for inner promise', function () {
		let canceled = 0;
	...
	it('cancelablePromise - cancel despite inner promise being resolved', function () {
		let canceled = 0;
		let promise = async.createCancelablePromise(token => {
			token.onCancellationRequested(_ => { canceled += 1; });
	...
	it('CancelablePromise execution order (sync)', function () {
		const order: string[] = [];
	...
	it('CancelablePromise execution order (async)', function () {
		const order: string[] = [];
	...
	it('cancelablePromise - get inner result', async function () {
		let promise = async.createCancelablePromise(token => {
	...
	it('Throttler - non async', function () {
		let count = 0;
		let factory = () => {
	...
	it('Throttler - last factory should be the one getting called', function () {
	...
	it('Delayer - simple cancel', function () {
		let count = 0;
		let factory = () => {
			return Promise.resolve(++count);
		};
	...
	it('Delayer - cancel should cancel all calls to trigger', function () {
		let count = 0;
		let factory = () => {
			return Promise.resolve(++count);
		};
	...
	it('Delayer - trigger, cancel, then trigger again', function () {
		let count = 0;
		let factory = () => {
			return Promise.resolve(++count);
		};
	...
	it('Delayer - last task should be the one getting called', function () {
		let factoryFactory = (n: number) => () => {
	...
	it('Limiter - assert degree of paralellism', function () {
	...
	it('Queue - order is kept', function () {
		let queue = new async.Queue();
	...
	it('Queue - errors bubble individually but not cause stop', function () {
		let queue = new async.Queue();
	...
	it('Queue - order is kept (chained)', function () {
		let queue = new async.Queue();
	...
	it('ResourceQueue - simple', function () {
	...
	// it.skip('TaskSequentializer - pending basics', async function () {
	...
	it('TaskSequentializer - pending and next (finishes instantly)', async function () {
		const sequentializer = new async.TaskSequentializer();
	...
	it('TaskSequentializer - pending and next (finishes after timeout)', async function () {
		const sequentializer = new async.TaskSequentializer();
	...
	it('TaskSequentializer - pending and multiple next (last one wins)', async function () {
		const sequentializer = new async.TaskSequentializer();
	...
	it('TaskSequentializer - cancel pending', async function () {
		const sequentializer = new async.TaskSequentializer();
