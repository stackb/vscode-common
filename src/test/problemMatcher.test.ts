...
import { IProblemReporter, ValidationState, ValidationStatus } from '../parsers';
import * as matchers from '../problemMatcher';
...
describe('ProblemPatternParser', () => {
	...
	describe('single-pattern definitions', () => {
		it('parses a pattern defined by only a regexp', () => {
		...
		it('does not sets defaults for line and character if kind is File', () => {
			let problemPattern: matchers.Config.ProblemPattern = {
				regexp: 'test',
				kind: 'file'
			};
	...
	describe('multi-pattern definitions', () => {
		it('defines a pattern based on regexp and property fields, with file/line location', () => {
		...
		it('defines a pattern bsaed on regexp and property fields, with location', () => {
			let problemPattern: matchers.Config.MultiLineProblemPattern = [
		...
		it('accepts a pattern that provides the fields from multiple entries', () => {
			let problemPattern: matchers.Config.MultiLineProblemPattern = [
				{ regexp: 'test', file: 3 },
				{ regexp: 'test1', line: 4 },
		...
		it('forbids setting the loop flag outside of the last element in the array', () => {
			let problemPattern: matchers.Config.MultiLineProblemPattern = [
		...
		it('forbids setting the kind outside of the first element of the array', () => {
			let problemPattern: matchers.Config.MultiLineProblemPattern = [
				{ regexp: 'test', file: 3 },
		...
		it('kind: Location requires a regexp', () => {
			let problemPattern: matchers.Config.MultiLineProblemPattern = [
		...
		it('kind: Location requires a regexp on every entry', () => {
			let problemPattern: matchers.Config.MultiLineProblemPattern = [
				{ regexp: 'test', file: 3 },
				{ line: 4 },
				{ regexp: 'test2', column: 5 },
		...
		it('kind: Location requires a message', () => {
			let problemPattern: matchers.Config.MultiLineProblemPattern = [
		...
		it('kind: Location requires a file', () => {
			let problemPattern: matchers.Config.MultiLineProblemPattern = [
		...
		it('kind: Location requires either a line or location', () => {
			let problemPattern: matchers.Config.MultiLineProblemPattern = [
		...
		it('kind: File accepts a regexp, file and message', () => {
			let problemPattern: matchers.Config.MultiLineProblemPattern = [
		...
		it('kind: File requires a message', () => {
			let problemPattern: matchers.Config.MultiLineProblemPattern = [
